...
<link type="text/css" href="${pageContext.request.contextPath }/resources/css/Header.css?v=<%=System.currentTimeMillis()%>" rel="stylesheet">
	...
	<% //로그인 상태에 따라 헤더부분 버튼을 다르게 노출함
		...
		if(id != null && id.equals("admin")){ %>
			...
			<div id="managerBtn"> <!-- 관리자 버튼 블록 -->
				<button class="btn4" onclick="location.assign('admin.do')">
					관리자 페이지</button>
			</div>
			<%}else{ %>
			<div id="managerBtn"> <!-- 관리자 버튼 블록 -->
				<button class="btn6" onclick="location.assign('main.do')">
					사용자 페이지</button>
			</div>
		<% }
				...
				<div id="btns"> <!-- 일반사용자 로그인 후버튼 블록 -->
					<button class="btn1" onclick="sessionCheck('hosting.do',id,doubleCheck);">
						마이호스팅</button>
					<button class="btn4" onclick="sessionCheck('myPage.do',id,doubleCheck);">
						마이페이지</button>
					<button class="btn5" onclick="location.assign('logout.do')">
						로그아웃</button>
				</div>
		...
		<% }else{ %>
			<div id="btns"> <!-- 일반사용자 로그인 전 버튼 블록 -->
				<button class="btn1" onclick="sessionCheck('hosting.do',id,doubleCheck)">
					마이호스팅</button>
				<button class="btn2" onclick="location.assign('login.do')">
					로그인</button>
				<button class="btn3" onclick="location.assign('joinForm.do')">
					회원가입</button>
			</div>
		<% }%>	
