...
<!-- int [] pageData = {page[0],begin[1],end[2],rowStart[3],rowEnd[4],totalCnt[5],rowCnt[6],steadyRowCnt[7],totalRow[8]} -->
		<c:forEach var="hostList" items="${hostList}" begin="${pageData[3] }" end="${pageData[4] }" step="1">
			<tr> 
...
<c:if test="${pageData[1] ne pageData[5]}">
	<div class="paging">
		<c:if test="${pageData[0] ne 1}">
			<span><a id="aTag" href="hostingList.do?pageIdx=1&steadyRowCnt=${pageData[7]}">&lt;&lt;</a></span>
			...
			<c:forEach varStatus="idx" begin="${pageData[1] }" end="${pageData[2] }" step="1">
				<c:if test="${pageData[0] eq idx.current }">
					<span><a id="currentPage" href="hostingList.do?pageIdx=${idx.current }&steadyRowCnt=${pageData[7]}">
					${idx.current }</a></span>
				</c:if>
				<c:if test="${pageData[0] ne idx.current }">
					<span><a id="aTag" href="hostingList.do?pageIdx=${idx.current }&steadyRowCnt=${pageData[7]}">
					${idx.current }</a></span>
				</c:if>
			</c:forEach>
